...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Search products by name (OCR/text recognition)
  app.get("/api/products/by-name/:name", isAuthenticated, async (req, res) => {
    try {
  ...
  // OCR endpoint for reading product names from images on the server (FAST)
  app.post("/api/products/recognize-from-image", isAuthenticated, imageUpload.single('image'), async (req: any, res) => {
    if (!req.file) {
    ...
    try {
      ...
      // Try to find product by extracted name
      const result = await storage.getAllProducts({
        limit: 100,
        offset: 0,
